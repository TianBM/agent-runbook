...
│   └── authoring-guide.md
├── runbooks/
│   ├── client-architecture-constraint-check.md
│   ├── git-pr-lifecycle.md
│   ├── opencode-doctor.md
│   ├── pr-watch-and-continue.md
│   ├── skill-quality-gate.md
│   ├── web-delivery-lifecycle.md
│   ├── web-project-quality-assessment.md
│   └── index.json
├── schemas/
│   └── runbook.schema.json
├── scripts/
│   ├── smoke-remote-runbooks.sh
...
更多字段语义和使用建议见 [Agent 远程查询说明](docs/agent-query.md)。它说明远程 agent 如何按 index-first 流程读取索引、选择 runbook、处理缓存和缺失字段。本项目不提供 runtime/search API，也不生成前端页面；搜索和筛选由调用方 agent 在本地完成。
...
- [Client architecture constraint check](runbooks/client-architecture-constraint-check.md)：覆盖 Web/App 客户端开发前的 API 合约、状态归属、有限值、宿主运行时和验证证据边界检查。
...
- [Web delivery lifecycle](runbooks/web-delivery-lifecycle.md)：覆盖本地验证、CI/CD、公网/内网部署路径和部署后证据闭环。
- [Web project quality assessment](runbooks/web-project-quality-assessment.md)：覆盖 Web 项目质量评估、技术选型识别、确定性检查、agent review 和 findings 分级。
